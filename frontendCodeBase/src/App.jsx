...
function Dashboard() {
  ...
  const fetchMetrics = async () => {
    try {
      ...
      const data = await res.json();
      setMetrics({
        db_file_size_mb: data.db_file_size_mb ?? data.db_size ?? 0,
        memory_percent: data.memory_percent ?? data.memory ?? 0,
        disk_percent: data.disk_percent ?? data.disk ?? 0,
        uptime_seconds: data.uptime_seconds ?? data.uptime ?? 0,
      });
    } catch (err) {
      console.warn("⚠️ Using fallback metrics", err);
    }
  };
  ...
  return (
    <div className="app">
      ...
      {/* Overlay ensures metrics appear above canvas */}
      <div className="overlay">
